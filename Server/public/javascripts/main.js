...
window.addEventListener("DOMContentLoaded", (event) => {
	...
	$(window).on("beforeunload", function () {
		if (uploadOnGoing) return confirm();
		return undefined;
	});
	//get public files
...
function getTableFiles(user) {
	if (!user) {
		getTableFiles("public");
		if (loggedIn) getTableFiles("private");
		return;
	}
	...
	$.get(`/files`, { username: filesOfUser }, (files) => {
            ...
            <tr fileid=${file.id}>
                ...
                <td>
                    <div class="btn-group-lg">
                        <button type="button" class="btn btn-default text-success" onclick="downloadFile('${
													file.id
												}')">
                            <span class="iconify icon-large" data-icon="mdi:download" data-inline="false"></span>
                        </button>
                        <button type="button" class="btn btn-default text-danger" onclick="deleteFile(event, '${
													file.id
												}')">
				...
				xhr.upload.addEventListener(
					...
					function (evt) {
						...
						if (evt.lengthComputable) {
							...
							$(`#uploadContainer .progress-bar span`).text(
								`${convertFileSize(evt.loaded)} / ${convertFileSize(evt.total)}`
							);
							$(`#uploadContainer .progress-bar`).width(percentComplete + "%");
							$(`.front`).css("clip-path", `inset(0 0 0 ${percentComplete}%)`);
							$(`.front`).css(
								"-webkit-clip-path",
								`inset(0 0 0 ${percentComplete}%)`
							);
							$("#uploadCurrentFileName").text(
								calculateCurrentFile(files, evt.loaded)
							);
						}
					},
					false
...
function registerUser(e) {
	...
	$.post(`/users/register`, registerFormData, function (data) {
		if (data) {
...
function loginUser(e) {
	...
	$.post(`/users/login`, loginFormData, function (data) {
		loginSuccess(data);
		$("#loginForm")[0].reset();
	});
}
...
function loginSuccess(data) {
	if (data) {
		...
		$("#loginButton").html(
			'Logout<span class="iconify icon-small" data-icon="mdi:logout" data-inline="false"></span>'
		);
...
function calculateCurrentFile(files, loaded) {
	...
	for (let file of files) {
		...
		if (loaded < totalBytes) return file.name;
	}
	return "--";
}
...
function setupIOSocket() {
	...
	socket.on("deleteFile", (fileId) => {
		const tableRow = $(`[fileid='${fileId}']`);
		tableRow.fadeOut(700, () => {
			tableRow.remove();
		});
	});
	socket.on("deleteAll", (table) => {
		const tableRows = $(`#${table}FileTable > tbody > tr`);
		tableRows.fadeOut(700, () => {
